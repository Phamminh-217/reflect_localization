...
## 4.7. `detection_loader.py`

### Trách nhiệm
Nạp và giải mã tệp tin kết quả phát hiện `detections.json` từ Phase 1 thành danh sách đối tượng `RFDetection` trong bộ nhớ theo từng khung hình.

### Đầu ra
Đối tượng cấu trúc `DetectionFrame`:
* `frame_index`: Chỉ số khung hình.
* `stamp`: Thời gian timestamp.
* `frame_id`: Tên hệ trục tọa độ LiDAR.
* `detections`: Danh sách các `RFDetection`.

---

## 4.8. `fallback_manager.py`

### Trách nhiệm
Quản lý trạng thái và dữ liệu Pose lịch sử gần nhất hợp lệ (`last_valid_pose`), quyết định áp dụng thuật toán dự phòng (fallback) để giữ cho quỹ đạo đầu ra của robot luôn liên tục không bị đứt quãng khi SVD tạm thời thất bại.

### Logic xử lý dự phòng
* Nếu kết quả định vị `status == OK`: Cập nhật `last_valid_pose` và reset đếm `consecutive_fallback_count = 0`.
* Nếu kết quả định vị `status != OK`:
  * Nếu tính năng fallback bật, có `last_valid_pose` lịch sử, và số khung hình lỗi liên tiếp dưới ngưỡng cấu hình: Trả về Pose fallback (cập nhật stamp mới) kèm trạng thái `FALLBACK_LAST_VALID_POSE` và cắm cờ `is_fallback = True`.
  * Nếu vượt ngưỡng tối đa: Trả về trạng thái lỗi gốc và `pose = None`.

---

## 4.9. `localization_writer.py`
...
## 4.10. `run_svd_localization.py` (scripts/)
...
## 4.11. `plot_localization_debug.py` (scripts/)
...
* Thêm đánh dấu trạng thái dự phòng (`Fallback marker`) nếu khung hình sử dụng lại pose fallback.
...
## 4.12. Luồng dữ liệu chi tiết của Phase 2.6 (Data Flow)
...
## 4.13. Yêu cầu tính truy vết Debug (Debug Traceability)
...
## 10. Output chuẩn với Fallback & Trace

Dưới đây là đặc tả chi tiết dòng dữ liệu mẫu cho các tệp kết xuất của Phase 2.6:

### `poses.csv`
`frame_index,stamp,status,x,y,yaw,num_matches,residual_rmse,max_residual,is_fallback,fallback_source,consecutive_fallback_count,reason`

* Ví dụ pose OK:
  `100,1716000000.100000,OK,1.2400,0.3500,0.0200,3,0.0150,0.0240,false,,0,Localization succeeded.`
* Ví dụ pose fallback:
  `101,1716000000.200000,FALLBACK_LAST_VALID_POSE,1.2400,0.3500,0.0200,3,0.0150,,true,last_valid_pose,1,Fallback applied: Insufficient detections: 2 < 3`
* Ví dụ frame fail không fallback:
  `106,1716000000.600000,INSUFFICIENT_DETECTIONS,,,,,,,false,,0,fallback limit exceeded`

---

### `rejected_frames.csv`
`frame_index,stamp,original_status,reason,num_detections,num_matches,fallback_used,fallback_status`

* Ví dụ:
  `101,1716000000.200000,INSUFFICIENT_DETECTIONS,Insufficient detections: 2 < 3,2,0,true,FALLBACK_LAST_VALID_POSE`
...
* Ví dụ:
  ...
  num_insufficient_detections,80
  num_association_failed,30
  num_degenerate_geometry,10
  num_high_residual,20
  ```

---

### `association_debug.csv`
`frame_index,stamp,detection_id,landmark_id,x_lidar,y_lidar,x_map,y_map,weight,residual`

---

### `svd_debug.csv`
`frame_index,stamp,R00,R01,R10,R11,tx,ty,yaw,det_R,residual_rmse,max_residual,num_points`

---

### `geometry_debug.csv`
`frame_index,stamp,num_pairs,spread_lidar,spread_map,condition_number_lidar,condition_number_map,warning,is_valid,is_degenerate`

---

### `frame_debug.csv`
`frame_index,stamp,num_detections,num_filtered_detections,num_matches,status,is_fallback,reason`

---

## 11. Roadmap module Phase 2.6

```text
2.6.1. pose.py (Cập nhật enum LocalizationStatus)
2.6.2. detection_loader.py (Nạp detections.json từ Phase 1)
2.6.3. fallback_manager.py (Duy trì pose liên tục bằng last_valid_pose)
2.6.4. localization_writer.py (Ghi 8 file output debug evidence đồng bộ)
2.6.5. run_svd_localization.py (CLI runner offline kết nối các khối)
2.6.6. plot_localization_debug.py (Script vẽ đồ thị debug trực quan)
```

Không nhảy cóc sang triển khai CLI runner trước khi tất cả các module loader, fallback, writer đều được bao phủ unit test đầy đủ.
...
## 13. Phase 2.7 — Real Bag End-to-End Validation Architecture

Để chạy định vị trên dữ liệu thực tế từ ROS bag, Phase 2.7 bổ sung cầu nối điều phối (pipeline bridge) và module đánh giá chất lượng pose (pose quality evaluator).

Sơ đồ kết nối:
```text
ROS Bag -> [run_phase1_to_phase2.py]
                 |
                 +---> [run_threshold_bag.py] (Phase 1) -> detections.json
                 |
                 +---> [run_svd_localization.py] (Phase 2) -> poses.csv + 7 debug files
                 |
                 +---> [generate_validation_report.py]
                            |
                            +---> [pose_evaluator.py] (Quality metrics)
                            |
                            +---> [plot_localization_debug.py] (PNG plots)
                            |
                            +---> validation_report.md
```

### Module bổ sung:
- **`pose_evaluator.py`**: Tính toán các chỉ số chất lượng định vị thực tế (OK rate, fallback rate, drift warning, streak fallback) trực tiếp từ `poses.csv` mà không cần ground truth.
- **`generate_validation_report.py`**: Tự động hóa việc tạo đồ thị trực quan và kết xuất báo cáo markdown `validation_report.md` chứa thông tin chẩn đoán vật lý.