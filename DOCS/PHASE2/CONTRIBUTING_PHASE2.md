...
## 13.7. Quy tắc bắt buộc cho Phase 2.7 (Validation & End-to-End)

### 1. Báo cáo tự động (Automatic validation report)
Mọi lần chạy thực nghiệm trên dữ liệu ROS bag thật đều phải được điều phối thông qua `run_phase1_to_phase2.py` để tự động hóa khâu phát hiện mốc, định vị SVD, vẽ đồ thị và xuất báo cáo markdown `validation_report.md` chứa phân tích chẩn đoán.

### 2. Ghi nhật ký thực địa
Khi chạy thành công trên một file bag thật mới, kỹ sư bắt buộc phải ghi lại kết quả tổng quan vào `DOCS/PHASE2/FIELD_VALIDATION_LOG.md` theo đúng mẫu quy định.
